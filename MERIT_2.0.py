...
def get_arg():
    ...
    parser = argparse.ArgumentParser(description=MERIT_description, prog='MERIT_v2.0.py',
...
class Step4(Steps):
    ...
    def __init__(self, args, step_index):
        ...
        self.set_output1(
            self.args.RES_dir + "/" + self.args.Name + "/" + self.args.Name + ".var.qual")  ### var.qual output
    ...
    def run(self):
        ...
        with open(self.args.RES_dir + "/" + self.args.Name + "/" + self.args.Name + ".mpileup", 'r') as self.fpileup:
            for self.line in self.fpileup:
                if (self.line[0:1] != "#"):
                    ...
                    self.dict_pileup['-'.join((self.cols_mp[0], self.cols_mp[1]))] = self.line.strip()

        ### Write header for var
        if ("2" in self.args.steps):
            if (self.args.ann):
                self.header_varqual = ["#chr", "pos", "ref", "ref_ctx", "alt", "alt_ctx", "total_depth_frw",
                                       "total_depth_rev", \
                                       "alt_depth_frw", "alt_depth_rev", "freq", "ave_alt_qual_frw", "ave_alt_qual_rev", \
                                       "pos_inread_alt_frw", "pos_inread_alt_rev", "indel_bases", "snp", "CDS",
                                       "effect"]
            else:
                self.header_varqual = ["#chr", "pos", "ref", "ref_ctx", "alt", "alt_ctx", "total_depth_frw",
                                       "total_depth_rev", \
        ...
        with open(self.input, 'r') as fvar:
            for self.line in fvar:
                if (self.line[0:1] != "#"):
                    var_line=self.line.strip().split("\t")
                    refvar=""
                    refvar=var_line[2]+var_line[4]
                    if(len(list(set(refvar))) == 1):
                        continue
...
class Step5(Steps):
    ...
    def __init__(self, args, step_index):
        ...
        self.set_input(
            self.args.RES_dir + "/" + self.args.Name + "/" + self.args.Name + ".var.qual")  ### var.qual input
        self.set_output1(
            self.args.RES_dir + "/" + self.args.Name + "/" + self.args.Name + ".var.ctx")  ### var.ctx output
        self.dashlist = list('-----------------------------------------------------')

        if ("2" in self.args.steps):
            if (self.args.ann):
                self.header_varctx = ["#chr", "pos", "ref", "ref_ctx", "alt", "alt_ctx", "total_depth_frw",
                                      "total_depth_rev", \
                                      "alt_depth_frw", "alt_depth_rev", "freq", "ave_alt_qual_frw", "ave_alt_qual_rev", \
            ...
            for self.line in fvarqual:
                if (self.line[0:1] == "#"):
                    ...
                    if (self.alt_chr != self.alt_chr_p):
                        ...
                        self.refseq = str(run_cmd(self.cmd_faidx, self.args.verbose, 1, "### Step 5 ###"))
                        self.refseq = self.refseq.replace(">chr{}".format(self.alt_chr), '').replace('\n', '').replace('\r', '')

                    ### SNV
                    if (self.lref == 1 and self.lalt == 1):
                        self.ref_cntx = list(map(str.upper, self.refseq[self.alt_pos - 2:self.alt_pos + 1]))
                    ...
                    if (self.lref > self.lalt):
                        ...
                        self.ref_cntx = list(map(str.upper, self.refseq[self.alt_pos - 1:self.alt_pos + self.lref]))
                        self.ref_cntx = ''.join(self.ref_cntx)
                        self.alt_cntx = self.ref_cntx[0] + (self.ref_cntx[1:self.lref]).replace(self.ind_bases, ''.join(
                            self.dashlist[0:len(self.ind_bases)]), 1) + self.ref_cntx[-1:]
                    ...
                    ### Insertion
                    if (self.lref < self.lalt):
                        self.alt_cntx = list(map(str.upper, self.refseq[self.alt_pos - 1:self.alt_pos + self.lref]))
                        self.alt_cntx = ''.join(self.alt_cntx)
                        self.alt_cntx = self.alt_cntx[0] + self.ind_bases + self.alt_cntx[1:]
                        self.ref_cntx = self.alt_cntx[0] + (self.alt_cntx[1:self.lalt]).replace(self.ind_bases, ''.join(
                            self.dashlist[0:len(self.ind_bases)]), 1) + self.alt_cntx[-1:]
                    ...
                    #############
                    self.varctxrow = self.cols[
                                     0:3] + self.ref_cntx.split() + self.altt.split() + self.alt_cntx.split() + self.cols[
                                                                                                                6:15] + self.ind_bases.split() + (
                                         str(self.num_homo_nt)).split() + self.cols[16:]
...
class Step6(Steps):
    ...
    def __init__(self, args, step_index):
        ...
        self.ind1nt = ['T', 'G', 'C', 'A']
        self.ind2nt = [['G', 'T'], ['G', 'C'], ['G', 'A'], ['A', 'T'], ['A', 'G'], ['A', 'C'], ['C', 'A'], ['C', 'G'],
                       ['C', 'T'], ['T', 'A'], ['T', 'C'], ['T', 'G']]
        ...
        with open(self.input, 'r') as self.fvarctx:
            for self.line in self.fvarctx:
                if (self.line[0:1] != "#"):
                    ...
                    if (len(self.cols[2]) == len(self.cols[4]) and float(self.cols[10]) < self.args.cut_off_freq and (
                            int(self.cols[6]) + int(self.cols[7])) > self.args.min_depth):
                        self.mykey = str(self.itr) + ',' + self.cols[3] + ',' + self.cols[5]
                        self.dctx_snv[self.mykey] = self.cols[0:]

                    if (len(self.cols[2]) < len(self.cols[4]) and float(self.cols[10]) < self.args.cut_off_freq and (
                            int(self.cols[6]) + int(self.cols[7])) > self.args.min_depth):
                        self.mykey = str(self.itr) + ',' + str(len(self.cols[15]) - 1) + ',' + self.cols[15] + ',HP:' + \
                                     self.cols[16] + '+'
    ...
    def run(self):
        ...
        for i_snv in range(96):
            ...
            self.mut_pos_tmp1 = list(map(float, self.myval[6:15]) for self.mkkey, self.myval in self.dctx_snv.items() if (str(self.ref_cntx) + ',' + str(self.alt_cntx)) in self.mkkey)
            self.mut_pos_tmp2 = list(map(float, self.myval[6:15]) for self.mkkey, self.myval in self.dctx_snv.items() if (str(self.ref_cntx_c) + ',' + str(self.alt_cntx_c)) in self.mkkey)
        ...
        for i_ind1 in range(len(self.ind1_mut_list)):
            ...
            ### INS
            self.mut_pos_ins1 = list(map(float, self.myval[6:15]) for self.mkkey, self.myval in self.dctx_ins.items() if (',+' + str(self.indel_nt) + ',HP:1+') in self.mkkey)
            ...
            ### DEL
            self.mut_pos_del1 = list(map(float, self.myval[6:15]) for self.mkkey, self.myval in self.dctx_del.items() if (',-' + str(self.indel_nt) + ',HP:1-') in self.mkkey)
        ...
        for kl in range(2, self.args.max_hp_no + 1):
            ...
            for ij in range(len(self.ind1_mut_list)):
                ...
                ### INS
                self.mut_pos_ins1 = list(map(float, self.myval[6:15]) for self.mkkey, self.myval in self.dctx_ins.items() if (',+' + str(self.indel_nt) + ',HP:' + str(kl) + '+') in self.mkkey)
                ...
                ### DEL
                self.mut_pos_del1 = list(map(float, self.myval[6:15]) for self.mkkey, self.myval in self.dctx_del.items() if (',-' + str(self.indel_nt) + ',HP:' + str(kl) + '-') in self.mkkey)
